...
var calendar = function(){
    ...
    var current_resource = function(){
    	return window.location.href.match(/subjects\/(\d+)\/bookings/)[1];
    };
    ...
    $('#calendar').fullCalendar({
        header: {
			...
			eventSources: [{  
    		url: '/subjects/'+current_resource()+'/bookings/',  
   		}],
    ...
    eventDrop: function(booking) {
      var length = (booking.end-booking.start)/(3600000);
        ...
        function updateEvent(booking) {
              $.ajax(
                '/subjects/'+current_resource()+'/bookings/'+booking.id,
                { 'type': 'PATCH',
    ...
    ,
      ...
      if (window.location.href.match(/new/)) {
        if(today_or_later()) {
          ...
          jQuery.post(
            '/subjects/'+current_resource()+'/bookings',
